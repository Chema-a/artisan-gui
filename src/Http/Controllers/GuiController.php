...
class GuiController extends Controller
{
    ...
    public function run($command)
    {
        ...
        try {
            ...
            $cleanOutput = preg_replace('#\\x1b[[][^A-Za-z]*[A-Za-z]#', '', $rawOutput);
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                $res = $decodedOutput;
                $res['status'] = $status;
                $res['command'] = $command->getName();

                if ($command->getName() === 'ghost_login' && isset($res['auto_redirect']) && $res['auto_redirect']) {
                    $res['auto_redirect_url'] = $res['redirect_url'];
                }
            } else {
                $res = [
                    'status'  => $status,
